...
async function loadNextEvent() {
  ...
  try {
    ...
    const {
      ...
      7000,
      'Le chargement du prochain événement a expiré.'
    );
...
// =========================================================
// PROTECTION CONTRE LES REQUÊTES QUI RESTENT BLOQUÉES
// =========================================================
...
document.addEventListener(
  ...
  () => {
    ...
    // Les éléments publics ne doivent surtout pas attendre
    // l'authentification : catalogue, événement et calendrier
    // se chargent indépendamment.
    loadNextEvent();
    loadGames();
    renderCalendar();

    // L'authentification est chargée en parallèle.
    withTimeout(
...
async function renderCalendar() {
  ...
  try {
    ...
    const {
      ...
      7000,
      'Le chargement du calendrier a expiré.'
    );
  ...
  try {
    ...
    const {
      ...
      supabase
        ...
        .order('name'),
      10000,
      'Le chargement du catalogue a expiré.'
    );
    ...
    // Le catalogue doit s'afficher même si les avis rencontrent
    // un problème réseau ou de permissions.
    renderGames();
    ...
    loadAllReviews()
      ...
      .catch(error => {
        console.error(
          'Erreur chargement avis en arrière-plan :',
          error
        );
      });
...
async function loadAllReviews() {
  ...
  try {
    ...
    const {
      ...
      7000,
      'Le chargement des avis a expiré.'
    );
